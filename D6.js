...
const str = function(string1,string2){
  ...
  const firtstring = string1.slice(0,2)
  const secondstring = string2.slice(-3)
  const result = firtstring + secondstring
 
  return result

  }

 console.log(str("studente", "riccardo").toUpperCase())
...
*/
const numeriarray =[3,6,9]
 const result = []
 for (let i = 0; i < numeriarray.length; i++) {
 result.push(numeriarray[i]  )}

 const finale = numeriarray.map((currentNumber) => {
